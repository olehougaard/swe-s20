...
public class OrderTest {
	...
	public void emptyOrderIsFree() {
		Assert.assertEquals(Money.ZERO, order.getPrice());
	}

	@Test
	public void emptyOrderHasNoOrderLines() {
		Assert.assertEquals(0, order.getLines().size());
	}
	
	@Test
	public void orderHasTheGivenId() {
		Assert.assertEquals(117, order.getId());
	}
	
	@Test
	public void orderHasTheGivenCurrency() {
		Assert.assertEquals("CHF", order.getCurrency());
	}
	...
	public void orderOfOneItemCostsTheCostOfTheItem() {
		order.add(1, item3);
		Assert.assertEquals(item3.getPrice(), order.getPrice());
	}
	
	@Test
	public void multipleCopiesOfSameItemCostsAmountTimesItemCost() {
		order.add(4, item3);
		Assert.assertEquals(item3.getPrice().multiply(4), order.getPrice());
	}
	...
	public void orderLineFromOrderOfOneItemHasAmountAndItem() {
		...
		Assert.assertEquals(4, orderLine.getAmount());
		Assert.assertEquals(item3, orderLine.getItem());
	}
	...
	public void removingLessThanTheFullAmountOfAnItemRetainsTheOrderLine() {
		...
		Assert.assertEquals(1, order.getLines().size());
		Assert.assertEquals(2, order.getLines().get(0).getAmount());
	}
	...
	public void multipleItemsCostTheSumOfTheItems() {
		order.add(1, item3);
		order.add(4, item12);
		Assert.assertEquals(item3.getPrice().add(item12.getPrice().multiply(4)), order.getPrice());
	}
	...
	public void repeatedAdditionsOfTheSameItemBecomesOneOrderLine() {
		...
		order.add(4, item3);
		Assert.assertEquals(1, order.getLines().size());
		Assert.assertEquals(6, order.getLines().get(0).getAmount());
	}
	...
	public void theOrderCannotBeExtendedThroughTheOrderLines() {
		...
		Assert.assertEquals(3, order.getLines().get(0).getAmount());
	}
